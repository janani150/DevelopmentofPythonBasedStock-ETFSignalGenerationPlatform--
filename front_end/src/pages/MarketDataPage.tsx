    ...
    const fetchMarketData = async () => {
      try {
        ...
        if (json.data) {
          ...
          const sortedLosers = [...json.data].sort(
        ...
        >
          ...
          <div className="space-y-3">
            {topGainers.map((s) => (
              <div
                key={s.symbol}
                className="flex items-center justify-between p-3 rounded-lg bg-gain/5 border border-gain/10"
              >
                <div>
                ...
                <div className="text-right">
                  ...
                  <div
                    className={`text-sm font-mono ${s.change >= 0 ? "text-gain" : "text-loss"}`}
                  >
                    {s.change > 0 ? "+" : ""}
                    {s.change.toFixed(2)}%
                  </div>
              ...
              <tbody>
                {filtered.map((s) => (
                  ...
                  >
                    ...
                    <td
                      className={`py-3 font-mono ${s.change >= 0 ? "text-gain" : "text-loss"}`}
                    >
                      {s.change >= 0 ? "+" : ""}
                      {s.change.toFixed(2)}%
                    </td>
