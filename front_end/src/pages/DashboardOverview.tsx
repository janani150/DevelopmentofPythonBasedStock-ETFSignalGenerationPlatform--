...
export default function DashboardOverview() {
    ...
    const fetchData = async () => {
      try {
        const [dashRes, portRes] = await Promise.all([
          fetch(`${API_BASE}/api/dashboard?email=${userEmail}`).then((r) =>
            r.json(),
          ),
          fetch(`${API_BASE}/api/portfolio?email=${userEmail}`).then((r) =>
            r.json(),
          ),
        ]);
  ...
  const stats = [
    {
      label: "Total Signals",
      value: dashboardData?.totalSignals?.toString() || "0",
      icon: Activity,
      color: "text-primary",
    },
    {
      label: "Active Buy",
      value: dashboardData?.activeBuy?.toString() || "0",
      icon: TrendingUp,
      color: "text-gain",
    },
    {
      label: "Active Sell",
      value: dashboardData?.activeSell?.toString() || "0",
      icon: TrendingDown,
      color: "text-loss",
    },
    {
      ...
      value: portfolioData?.totalReturnPercent
        ? `${portfolioData.totalReturnPercent > 0 ? "+" : ""}${portfolioData.totalReturnPercent.toFixed(1)}%`
        : "0.0%",
      icon: BarChart3,
      color: "text-gain",
    },
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {stats.map((s, i) => (
          <motion.div key={s.label} {...anim(i)} className="stat-card">
            ...
            <AnimatedCounter
              value={s.value}
              className="stat-value text-lg md:text-2xl text-foreground"
            />
          </motion.div>
        ))}
      </div>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie
                ...
                cy="50%"
              >
                {(dashboardData?.signalDistribution || []).map(
                  (entry: any, i: number) => (
            ...
            <tbody>
              {(dashboardData?.recentSignals || []).map((s: any) => (
                ...
                >
                  ...
                  <td
                    className={`py-3 font-mono ${s.changePercent >= 0 ? "text-gain" : "text-loss"}`}
                  >
                    {s.changePercent >= 0 ? "+" : ""}
